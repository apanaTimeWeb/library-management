...
This document serves as the canonical map for the `/app/manager` module. It was completely refactored to adhere to extreme micro-modularization, AI-Friendly logic isolation, and Enterprise-Grade frontend standards.
...
2. **Absolute Imports Only**: Use `@/app/manager/...` exclusively. Never use relative paths like `../` or `./`. (Rule 10)
...
4. **Logic Isolation (Rule 6)**: Complex state (`useState`, `useEffect`, API fetches) is strictly extracted into custom hooks (e.g., `useManagerCrmEnquiriesDetail.ts`) inside `[moduleName]_hooks/`. The `.tsx` components act as pure View layers.
5. **Component Contracts (Rule 38, Rule 39, Rule 55)**:
   - All UI components must begin with `// RESPONSIBILITY: [description]`.
   - All custom hooks must begin with `// DATA FLOW: [description]`.
   - All `useEffect` blocks must be preceded by `// DEPENDENCY AUDIT: [explanation]`.
6. **Centralized Routing**: No hardcoded URLs in components. Import from `manager_url_config.ts`.
7. **No Barrel Files**: `index.ts` files are strictly forbidden to avoid circular dependencies (Rule 18).

## Sub-Module Directory Map

The `/app/manager` module is divided into highly cohesive feature sub-modules:
...
- **`manager_students_hooks/`**: Data fetching and filtering (`useManagerStudentsIdCard.ts`).
...
- **`manager_seats_shifts_lockers_components/`**: UI Views (`ManagerSeatsLockerMatrixClient.tsx`).
- **`manager_seats_shifts_lockers_hooks/`**: State/API logic (`useManagerSeatsAllocations.ts`).
...
- **`manager_shared_hooks/`**: Contains cross-module hooks like `useManagerDebounce.ts`.
...
- **Dependencies**: Never use `ag-grid`. We use native `<table>` structures. Never use inline `#hex` colors in Tailwind; use CSS variables mapped in `tailwind.config.ts`.
