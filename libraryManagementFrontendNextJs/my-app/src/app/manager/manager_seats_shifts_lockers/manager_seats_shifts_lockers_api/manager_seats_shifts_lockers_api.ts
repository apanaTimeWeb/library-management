...
import { SeatData, LockerData, Allocation, SeatHistoryEntry } from '@/app/manager/manager_seats_shifts_lockers/manager_seats_shifts_lockers_types/ManagerSeatsTypes';
...
export async function fetchAllocations(): Promise<Allocation[]> {
  return new Promise(resolve => setTimeout(() => resolve(MOCK_ALLOCATIONS as unknown as Allocation[]), 500));
}
