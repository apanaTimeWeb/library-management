  ...
  return (
    <div className="bg-card rounded-xl border border-border p-6 h-full flex flex-col">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-base font-semibold text-text-primary">Mini Seat Matrix â€” Today&apos;s Shifts</h2>
      </div>
      <div className="flex flex-col flex-1 justify-between">
        <div className="grid grid-cols-5 sm:grid-cols-8 gap-2 mb-6">
          {seatData.map((seat) => (
            <div
              key={seat.id}
              className={`aspect-square rounded-md border flex items-center justify-center text-xs font-bold cursor-pointer transition-colors hover:opacity-80 ${SEAT_CLASS[seat.status] || ''}`}
        ...
        <div className="flex flex-wrap items-center justify-center gap-4 border-t border-border pt-4 mt-auto">
