...
export function ManagerCrmEnquiriesKanban({ isEmpty, getCardsByStatus, onAddEnquiry }: ManagerCrmEnquiriesKanbanProps) {
  ...
  if (isEmpty) {
    return (
      <div className="flex flex-col items-center justify-center bg-card border border-border rounded-lg p-12 text-center mt-12">
          ...
          <div key={col.id} className="flex-shrink-0 w-80 flex flex-col gap-3">
            <div className="flex items-center justify-between pb-2 border-b border-border">
              <div className="flex items-center gap-2">
                ...
                <span className="bg-card text-text-secondary text-xs font-medium px-2 py-0.5 rounded-full">{cards.length}</span>
              </div>
              <button className="p-1 text-text-secondary hover:text-text-primary transition-colors rounded-md hover:bg-card" title="More options" aria-label="Column options">
