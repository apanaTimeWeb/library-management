...
function getShiftClass(shift: string): string {
  ...
  return map[shift] ?? 'bg-bg-pageg-input text-text-primary';
}
...
export function AdminDashboardSeatCell({ id, status, occupant, shift, expiry, onClick }: AdminDashboardSeatCellProps) {
  const statusClasses = {
    ...
    maintenance: 'bg-bg-pageg-page text-text-secondary border-border hover:bg-bg-pageg-input',
  };
  ...
  return (
    ...
    >
      ...
      {/* Tooltip */}
      <div className="absolute z-10 bottom-full mb-2 left-1/2 -translate-x-1/2 w-max max-w-52 bg-bg-pageg-card border border-border text-text-primary text-xs rounded-lg px-3 py-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all shadow-xl flex flex-col items-center gap-1">
        {occupant ? (
          <>
