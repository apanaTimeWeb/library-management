'use client';
// RESPONSIBILITY: Renders the AdminSystemDataExportClient component.
...
import { ADMIN_SYSTEM_QUICK_EXPORTS } from '@/app/admin/admin_system/admin_system_utils/AdminSystemMockData2';
...
export function AdminSystemDataExportClient() {
  const {
    ...
    estimatedTotal, modules
  } = useAdminSystemDataExport();
    ...
    <div>
      ...
      <Card>
        ...
        <CardContent className="space-y-5">
                ...
                <div
                  ...
                  className={`flex items-start gap-3 p-3.5 rounded-xl border cursor-pointer transition-all ${
                    isSelected
                      ...
                      : 'border-border hover:border-primary/25 hover:bg-bg-pageg-card'
                  }`}
                >
          ...
          {/* Filters row */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 rounded-xl bg-bg-pageg-card border border-border">
                  ...
                  <button
                    ...
                    className={`flex-1 py-2 rounded-lg text-sm font-semibold border transition-all ${
                      format === f
                        ...
                        : 'bg-bg-pageg-card text-text-secondary border-border hover:border-primary/40'
                    }`}
                  >
                    {f}
                  </button>
            ...
            <div className="space-y-1.5">
              ...
              <input
                ...
                className="w-full px-3 py-2 rounded-lg bg-bg-pageg-card border border-border text-sm text-text-primary focus:outline-none focus:border-primary"
              />
            </div>
            <div className="space-y-1.5">
        ...
        <CardFooter>
          <div className="flex items-center gap-4 w-full">
            ...
            >
              {exporting === 'bulk'
                ? <><Loader2 size={16} className="animate-spin mr-1" /> Exporting...</>
                : exported.has('bulk')
                ? <><CheckCircle size={16} className="mr-1" /> Downloaded!</>
                : <><Download size={16} className="mr-1" /> Export {selected.size > 0 ? `${selected.size} Module${selected.size > 1 ? 's' : ''}` : 'Data'}</>}
            </Button>
          </div>
        </CardFooter>
      </Card>
