...
export function AdminFinanceRefundsClient() {
  ...
  return (
    <div className="h-full flex flex-col pb-10 space-y-6 relative">
              ...
              {isLoading ? (
                ...
                table.paginatedData.map((r) => (
                  <tr key={r.id} className="hover:bg-muted/10 transition-colors">
                    ...
                    <td className="px-5 py-4 text-right">
                      <span className={`font-bold text-sm ${r.deductionAmount > 0 ? 'text-danger' : 'text-muted-foreground'}`}>
                        {r.deductionAmount > 0 ? `-${formatCurrency(r.deductionAmount)}` : 'None'}
                      </span>
                    </td>
                    <td className="px-5 py-4 text-right">
                      <span className="font-bold text-sm text-success">{formatCurrency(r.netRefund)}</span>
                    </td>
                    <td className="px-5 py-4">
      ...
      {processDialog && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-bg-pagelack/60 backdrop-blur-sm" onClick={() => setProcessDialog(null)}>
      ...
      {/* Add Deduction Modal */}
      {deductDialog && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-bg-pagelack/60 backdrop-blur-sm" onClick={() => setDeductDialog(null)}>
