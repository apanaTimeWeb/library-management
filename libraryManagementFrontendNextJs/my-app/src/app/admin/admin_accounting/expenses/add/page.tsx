...
export default function AddExpensePage() {
  ...
  const [form, setForm] = useState({ date: '', category: ADMIN_ACCOUNTING_EXPENSES_CATEGORIES[0], description: '', amount: '', paidBy: '', mode: 'cash', notes: '' });
  ...
  return (
    <div className="space-y-6 max-w-xl">
      ...
      <div className="fin-card p-6 space-y-4">
        ...
        <div><label className="fin-label">Category</label><select className="fin-select" value={form.category} onChange={e => setForm(p=>({...p,category:e.target.value}))}>{ADMIN_ACCOUNTING_EXPENSES_CATEGORIES.map(c=><option key={c}>{c}</option>)}</select></div>
