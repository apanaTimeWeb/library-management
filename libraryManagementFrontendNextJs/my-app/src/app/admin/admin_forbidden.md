...
- **NEVER** import anything from another feature module (e.g., `manager`, `superadmin`, `student`).
- **NEVER** hardcode routes to other modules (e.g., `/manager/manager_students`). Use `ADMIN_ROUTES` from `admin_url_config.ts`.
- **NEVER** use barrel files (`index.ts`). Always import directly from the named file.
- **NEVER** use relative imports (`../../`). Always use `@/app/admin/...`.

## TypeScript (Rules 27, 60)
- **NEVER** use the `any` type. Use `unknown` and validate with Zod.
- **NEVER** use `AdminRecord` as a type workaround — define proper typed interfaces.
...
- **NEVER** hardcode URL strings (e.g., `'/crm/enquiries'`) in components or hooks.
- All routes must be imported from `admin_url_config.ts`.
...
- **NEVER** use hardcoded hex values in `.tsx` files — all hex lives in `admin.css`.
