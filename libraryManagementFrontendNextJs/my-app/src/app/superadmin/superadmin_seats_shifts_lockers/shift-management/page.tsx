...
export default function ShiftManagementPage() {
  ...
  function handleSave() {
    if (!validate()) return;
    if (editShift) {
      setShifts(prev => prev.map(( s: FlexRecord ) => s.id === editShift.id ? { ...s, ...form } : s));
  ...
  function handleDeactivate() {
    ...
    setShifts(prev => prev.map(( s: FlexRecord ) => s.id === deactivateTarget.id ? { ...s, active: false } : s));
    toast.success(`${deactivateTarget.name} shift deactivated.`);
    setDeactivateTarget(null);
  }

  function handleActivate(shift: Shift) {
    setShifts(prev => prev.map(( s: FlexRecord ) => s.id === shift.id ? { ...s, active: true } : s));
