      ...
      <table className="w-full text-left border-collapse">
        <thead>
          <tr className="bg-bg-page/50 border-b border-border">
            <th className="py-3.5 px-4 text-[11px] font-bold text-text-disabled uppercase tracking-wider">Date</th>
          ...
          {table.paginatedData.map((d, i) => (
            <tr key={i} className="hover:bg-bg-page/30 transition-colors">
              ...
              <td className="py-3.5 px-4">
                <span className="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider bg-bg-input border border-border text-text-secondary shadow-sm">
                  {d.shift}
                </span>
              </td>
              <td className="py-3.5 px-4 text-sm font-extrabold text-text-primary">{d.seatNo}</td>
              <td className={`py-3.5 px-4 text-[15px] font-extrabold tracking-tight text-right ${d.gapDays > 20 ? 'text-danger' : 'text-warning'}`}>
                {d.gapDays}d
              </td>
              <td className="py-3.5 px-4 text-[15px] font-extrabold text-danger tracking-tight text-right">₹{d.loss.toLocaleString()}</td>
            </tr>
          ))}
        </tbody>
      </table>
