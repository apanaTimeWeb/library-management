  ...
  return (
    <div className="space-y-6 max-w-xl">
      ...
      <div className="fin-card p-6 space-y-4">
        ...
        <div><label className="fin-label">Category</label><select className="fin-select" value={form.category} onChange={e => setForm(p=>({...p,category:e.target.value}))}>{CATEGORIES.map(( c: string ) =><option key={c}>{c}</option>)}</select></div>
