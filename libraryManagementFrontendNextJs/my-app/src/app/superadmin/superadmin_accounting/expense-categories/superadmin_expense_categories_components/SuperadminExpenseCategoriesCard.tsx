  ...
  return (
    <div className="bg-card border border-border rounded-lg p-5 shadow-sm flex flex-col gap-4">
      <div className="flex items-center justify-between">
        ...
        <button 
          className="w-7 h-7 flex items-center justify-center rounded-md bg-danger-bg text-danger hover:bg-danger hover:text-white transition-colors duration-200" 
      ...
      <div className="flex items-center justify-between">
        <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-bold uppercase tracking-wider ${over ? 'bg-danger-bg text-danger' : pct > 80 ? 'bg-warning-bg text-warning' : 'bg-success-bg text-success'}`}>
          {pct}% used
        </span>
        {over && (
