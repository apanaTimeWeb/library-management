...
export function SuperadminKpiCard({ title, value, subtitle, icon, trend, trendLabel, className }: SuperadminKpiCardProps & { subtitle?: string; trend?: 'up' | 'down' | 'neutral'; trendLabel?: string; className?: string }) {
  ...
  return (
    <div className={cn('bg-card border border-border rounded-lg p-4 shadow-sm flex flex-col', className)}>
      ...
      <div className="text-3xl font-extrabold text-text-primary leading-none mt-3">{value}</div>
      {(subtitle || trend) && (
