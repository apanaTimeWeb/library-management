...
export function SuperadminDashboardRecentActivity() {
  return (
    <div className="bg-bg-card border border-border rounded-[var(--radius-lg)] overflow-hidden shadow-sm mt-8">
      ...
      <div className="divide-y divide-[var(--border)]">
        {SUPERADMIN_DASHBOARD_RECENT_ACTIVITY.map((item, i) => (
          <Link key={i} href={item.href} className="block p-4 hover:bg-bg-input transition-colors duration-200">
            <div className="flex items-center gap-4">
              ...
              <span className="text-[11px] font-medium text-text-disabled shrink-0">{item.time}</span>
            </div>
          </Link>
        ))}
      </div>
