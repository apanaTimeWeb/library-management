...
export function SuperadminDashboardKpiCard({ title, value, icon, subtitle, trend, progress, alert }: KpiCardProps) {
  return (
    <div className="bg-bg-card border border-border rounded-[var(--radius-lg)] p-5 flex flex-col justify-between shadow-sm">
      ...
      <div className="mt-4">
        ...
        <div className="mt-2 flex items-center gap-2 flex-wrap">
          {trend && (
            <span className="flex items-center gap-1 text-[11px] font-bold text-success bg-[var(--success-bg,rgba(52,211,153,0.1))] px-2 py-0.5 rounded-full">
              <TrendingUp size={11} /> {trend}
            </span>
          )}
          {subtitle && <span className="text-xs text-text-disabled">{subtitle}</span>}
          {alert && (
            <span className="flex items-center gap-1 text-[11px] font-bold text-danger bg-[var(--danger-bg,rgba(248,113,113,0.1))] px-2 py-0.5 rounded-full">
              <AlertTriangle size={11} /> {alert}
            </span>
          )}
          {progress !== undefined && (
            <div className="h-1.5 w-16 bg-bg-input rounded-full overflow-hidden">
