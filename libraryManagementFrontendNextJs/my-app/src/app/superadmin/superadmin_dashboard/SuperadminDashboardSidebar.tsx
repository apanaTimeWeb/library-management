  ...
  return (
    <aside className={`fixed md:static inset-y-0 left-0 z-40 w-64 bg-bg-pageg-card border-r border-border flex flex-col transition-transform ${open ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
      <div className="flex items-center gap-3 p-5 shrink-0">
        ...
        <div>
          <p className="font-bold text-base text-text-primary leading-tight">Nexus 360</p>
          <p className="text-xs font-medium text-text-secondary uppercase tracking-wider">Super Admin Panel</p>
        </div>
      </div>
      ...
      <div className="p-4 border-t border-border bg-bg-pageg-card flex items-center gap-3 shrink-0">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-text-secondary truncate">Platform Owner</p>
        </div>
        <button
      ...
      {showLogout && (
        <div className="fixed inset-0 bg-bg-pagelack/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setShowLogout(false)}>
          <div className="bg-bg-pageg-card max-w-[360px] w-full rounded-2xl shadow-2xl p-6 relative border border-border flex flex-col items-center text-center animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
