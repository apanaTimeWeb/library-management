          ...
          <div className="grid grid-cols-2 gap-4 bg-bg-page rounded-[var(--radius-md)] p-4 border border-border">
            {[['Billing Cycle',sub.cycle],['Start Date',sub.startDate],['Next Invoice',sub.nextInvoice],['Seats',`${sub.seats} seats`]].map(([label,val]) => (
              <div key={label} className="flex flex-col gap-1">
                <p className="text-[11px] font-bold text-text-disabled uppercase tracking-wider">{label}</p>
                <p className="text-sm font-medium text-text-primary">{val}</p>
              </div>
            ))}
          </div>

          <div className="bg-bg-input rounded-[var(--radius-md)] p-5 border border-border flex items-center justify-between shadow-inner">
            <div className="flex items-center gap-2 text-text-secondary">
              <IndianRupee size={16} /> <span className="text-sm font-bold uppercase tracking-wider">Monthly Value</span>
            </div>
            <span className="text-2xl font-extrabold text-primary tracking-tight">₹{sub.mrr.toLocaleString()}</span>
          </div>
          ...
          {editing ? (
            <div className="space-y-4 pt-2">
              ...
              <div className="flex gap-3 pt-2">
                <button 
                  className="flex-1 flex items-center justify-center gap-2 bg-primary hover:bg-primary-hover text-white text-sm font-bold py-2.5 px-4 rounded-[var(--radius-md)] transition-all disabled:opacity-70 disabled:cursor-not-allowed shadow-[0_0_10px_rgba(99,102,241,0.2)]" 
                ...
                <button 
                  className="flex-1 flex items-center justify-center gap-2 bg-transparent border border-border hover:bg-bg-input text-text-primary text-sm font-bold py-2.5 px-4 rounded-[var(--radius-md)] transition-colors" 
