...
export function AbsenteeReportClient() {
  ...
  const badgeClass = (d: number) => d >= 7 ? 'bg-[#450A0A] text-[#F87171]' : 'bg-[#451A03] text-[#FBBF24]';
  const getRowClass = (d: number) => {
    if (d >= 7) return 'bg-[#450A0A]/50 hover:bg-[#450A0A]/70';
    if (d >= 3) return 'bg-[#451A03]/30 hover:bg-[#451A03]/50';
      ...
      <div className="bg-bg-card border border-border rounded-t-lg p-4 border-b">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap gap-2">
            <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold bg-[#450A0A] text-[#F87171]">{critical.length} critical</span>
            <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold bg-[#451A03] text-[#FBBF24]">{moderate.length} moderate</span>
            <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold bg-[#064E3B] text-[#34D399]">{filtered.filter(r=>r.notified).length} notified</span>
          </div>
        </div>
      </div>
                      ...
                      >
                        ...
                        <TableCell>
                          <div className="h-full flex items-center">
                            {a.notified ? (
                              <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-bold bg-[#064E3B] text-[#34D399]"><CheckCircle size={12} /> Notified</span>
                            ) : (
