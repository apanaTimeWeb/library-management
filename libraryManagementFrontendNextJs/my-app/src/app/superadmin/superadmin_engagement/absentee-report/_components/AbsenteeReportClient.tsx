    ...
    { 
      ...
      cellRenderer: (p: ICellRendererParams) => (
        <div className="flex items-center py-2 h-full">
          ...
          <span className="text-sm font-bold text-text-primary truncate">{p.value}</span>
        </div>
      )
    },
    { field: 'smartId', headerName: 'Smart ID', width: 120, cellRenderer: (p: ICellRendererParams) => <span className="font-mono text-sm text-text-secondary">{p.value}</span> },
    { field: 'shift', headerName: 'Shift', width: 120, cellRenderer: (p: ICellRendererParams) => <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-bold bg-input text-text-secondary mt-2 inline-block">{p.value}</span> },
    { 
      field: 'daysAbsent', 
      headerName: 'Days Absent', 
      width: 140,
      cellRenderer: (p: ICellRendererParams) => (
        <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-bold ${badgeClass(p.value)} mt-2 inline-block`}>
          {p.value} days
        </span>
      )
    },
    { field: 'lastSeen', headerName: 'Last Seen', width: 130, cellRenderer: (p: ICellRendererParams) => <span className="text-sm text-text-secondary">{p.value}</span> },
    { 
      field: 'parentPhone', 
      ...
      cellRenderer: (params: ICellRendererParams) => (
        <div className="h-full flex items-center">
          {params.data.notified ? (
            <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-bold bg-success/10 text-success"><CheckCircle size={12} /> Notified</span>
          ) : (
            <button onClick={() => notify(params.data.id)} className="flex items-center gap-1.5 px-3 py-1 bg-transparent border border-border text-text-primary text-xs font-bold rounded-md hover:bg-primary hover:text-primary-foreground hover:border-primary transition-all duration-200 cursor-pointer">
  ...
  return (
    <div className="relative p-2 sm:p-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <div className="bg-card border border-border rounded-lg p-4 shadow-sm flex flex-col items-center text-center">
      ...
      <div className="bg-card border border-border rounded-t-[var(--radius-lg)] p-4 border-b">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap gap-2">
            <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold bg-danger/10 text-danger">{critical.length} critical</span>
            <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold bg-warning/10 text-warning">{moderate.length} moderate</span>
            <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold bg-success/10 text-success">{filtered.filter(r=>r.notified).length} notified</span>
          </div>
        </div>
      </div>
