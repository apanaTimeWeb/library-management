    ...
    <div className="fixed inset-0 z-50 flex items-stretch justify-end" onClick={onClose}>
      ...
      >
        <div className="p-6 space-y-6">
          <div className="flex items-start justify-between">
            <div>
              <div className="flex items-center gap-2 mb-1.5">
                ...
                <span className="text-[13px] font-bold text-text-secondary tracking-wide">{inv.id}</span>
              </div>
          ...
          <div className="bg-bg-page rounded-[var(--radius-lg)] p-6 border border-border text-center shadow-inner">
            <p className="text-[11px] font-bold text-text-disabled uppercase tracking-wider mb-2">Invoice Amount</p>
          ...
          <div className="grid grid-cols-2 gap-4">
            {[['Date',inv.date],['Method',inv.method],['GST Number',inv.gst],['Status',inv.status]].map(([label,val]) => (
              <div key={label} className="bg-bg-input p-3 rounded-[var(--radius-md)] border border-border flex flex-col gap-1">
                <p className="text-[10px] font-bold text-text-disabled uppercase tracking-wider">{label}</p>
                <p className="text-[13px] font-bold text-text-primary truncate">{val}</p>
              </div>
            ))}
          </div>
          ...
          <div className="flex gap-3 pt-2 flex-wrap border-t border-border mt-4 pt-6">
            <button 
              className={`flex-1 flex items-center justify-center gap-2 text-white text-sm font-bold py-2.5 px-4 rounded-[var(--radius-md)] transition-all disabled:opacity-70 disabled:cursor-not-allowed shadow-[0_0_10px_rgba(99,102,241,0.2)] ${downloaded ? 'bg-success hover:bg-success-hover' : 'bg-primary hover:bg-primary-hover'}`} 
