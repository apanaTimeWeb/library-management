...
export function NoticesClient() {
  ...
  const colDefs = [
    ...
    { field: 'message', headerName: 'Message', flex: 1, minWidth: 250, cellRenderer: (p: ICellRendererParams) => <span className="text-text-secondary text-sm truncate block w-full pt-1" title={p.value}>{p.value}</span> },
    { 
      field: 'status', 
      headerName: 'Status', 
      width: 120,
      cellRenderer: (p: ICellRendererParams) => (
        <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-bold ${p.value === 'Active' ? 'bg-success/10 text-success' : 'bg-input text-text-secondary'} mt-2 inline-block`}>
          {p.value === 'Active' ? 'Active' : 'Expired'}
        </span>
      )
    },
    { field: 'validTill', headerName: 'Valid Till', width: 130, cellRenderer: (p: ICellRendererParams) => <span className="font-mono text-sm text-text-secondary">{p.value}</span> },
    { field: 'postedBy', headerName: 'Posted By', width: 130, cellRenderer: (p: ICellRendererParams) => <span className="text-text-secondary text-sm">{p.value}</span> },
    { field: 'postedDate', headerName: 'Posted Date', width: 130, cellRenderer: (p: ICellRendererParams) => <span className="font-mono text-sm text-text-secondary">{p.value}</span> },
    {
      headerName: 'Actions',
      width: 140,
      sortable: false,
      cellRenderer: (params: ICellRendererParams) => (
        <div className="h-full flex items-center gap-2">
          <button onClick={() => openEdit(params.data)} className="w-7 h-7 flex items-center justify-center rounded-md text-text-secondary hover:bg-primary hover:text-primary-foreground transition-colors duration-200 cursor-pointer" title="Edit"><Edit2 size={14} /></button>
          <button onClick={() => setBroadcastItem(params.data)} className="w-7 h-7 flex items-center justify-center rounded-md text-text-secondary hover:bg-primary hover:text-primary-foreground transition-colors duration-200 cursor-pointer" title="Broadcast"><Send size={14} /></button>
          <button onClick={() => setDeleteItem(params.data)} className="w-7 h-7 flex items-center justify-center rounded-md text-danger hover:bg-danger hover:text-danger-foreground transition-colors duration-200 cursor-pointer" title="Delete"><Trash2 size={14} /></button>
        </div>
      )
    }
  ];
  ...
  return (
    <div className="relative p-2 sm:p-4">
      ...
      {/* Add/Edit Modal */}
      {showAdd && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-bg-pagelack/50 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-card w-full max-w-lg rounded-xl shadow-2xl border border-border flex flex-col relative overflow-hidden">
            ...
            <div className="p-6 space-y-4">
              <div>
                ...
                <input className="w-full bg-input border border-border rounded-md py-2.5 px-3 text-sm font-medium text-text-primary focus:outline-none focus:border-primary transition-colors" placeholder="Notice title"
              ...
              <div>
                ...
                <textarea className="w-full bg-input border border-border rounded-md py-2.5 px-3 text-sm font-medium text-text-primary focus:outline-none focus:border-primary transition-colors" rows={6} placeholder="Notice message..."
              ...
              <div>
                ...
                <input type="date" className="w-full bg-input border border-border rounded-md py-2.5 px-3 text-sm font-medium text-text-primary focus:outline-none focus:border-primary transition-colors"
            ...
            <div className="px-6 py-4 bg-muted border-t border-border flex justify-end gap-3 rounded-b-[var(--radius-xl)]">
              <button onClick={() => setShowAdd(false)} className="px-4 py-2 border border-border text-text-primary text-sm font-bold rounded-md hover:bg-input transition-colors cursor-pointer">Cancel</button>
      ...
      {deleteItem && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-bg-pagelack/50 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-card w-full max-w-sm rounded-xl shadow-2xl border border-border flex flex-col p-6">
            ...
            <div className="flex justify-end gap-3">
              <button onClick={() => setDeleteItem(null)} className="px-4 py-2 border border-border text-text-primary text-sm font-bold rounded-md hover:bg-input transition-colors cursor-pointer">Cancel</button>
      ...
      {broadcastItem && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-bg-pagelack/50 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-card w-full max-w-sm rounded-xl shadow-2xl border border-border flex flex-col p-6">
            ...
            <div className="flex justify-end gap-3">
              <button onClick={() => setBroadcastItem(null)} className="px-4 py-2 border border-border text-text-primary text-sm font-bold rounded-md hover:bg-input transition-colors cursor-pointer">Cancel</button>
