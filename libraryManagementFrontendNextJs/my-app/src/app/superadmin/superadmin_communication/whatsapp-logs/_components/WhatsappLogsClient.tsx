...
export function WhatsappLogsClient() {
  ...
  const colDefs = [
    { field: 'dateTime', headerName: 'Date / Time', width: 160, cellRenderer: (p: ICellRendererParams) => <span className="text-text-secondary text-sm">{p.value}</span> },
    { field: 'phone', headerName: 'Phone', width: 130, cellRenderer: (p: ICellRendererParams) => <span className="font-mono font-medium text-sm text-text-primary">{p.value}</span> },
    ...
    { 
      ...
      cellRenderer: (p: ICellRendererParams) => (
        <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-bold ${TYPE_BADGE[p.value] || 'bg-input text-text-secondary'} mt-2 inline-block`}>
          {TYPE_LABEL[p.value]}
        </span>
      )
    },
    { 
      ...
      width: 120,
      cellRenderer: (p: ICellRendererParams) => (
        <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-bold ${STATUS_BADGE[p.value] || 'bg-input text-text-secondary'} mt-2 inline-block`}>
          {p.value}
        </span>
      )
    },
    { field: 'error', headerName: 'Error', width: 180, cellRenderer: (p: ICellRendererParams) => <span className="text-danger font-medium text-xs truncate max-w-xs inline-block" title={p.value}>{p.value || '—'}</span> }
  ];
          ...
          <div className="bg-card w-full max-w-lg rounded-xl shadow-2xl border border-border flex flex-col relative overflow-hidden">
            ...
            <div className="p-6">
              <div className="mb-6 flex gap-2">
                <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-bold ${TYPE_BADGE[viewLog.type]}`}>{TYPE_LABEL[viewLog.type]}</span>
                <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-bold ${STATUS_BADGE[viewLog.status]}`}>{viewLog.status}</span>
              </div>
              <div className="grid grid-cols-2 gap-4 mb-6 bg-input p-4 rounded-lg">
              ...
              {viewLog.error && (
                <div className="mt-4 p-3 bg-danger/10 text-danger rounded-lg border border-danger/20 text-sm font-bold flex items-center gap-2"><AlertTriangle size={14} /> Error: {viewLog.error}</div>
              )}
            </div>
            <div className="px-6 py-4 bg-muted border-t border-border flex justify-end">
              <button onClick={() => setViewLog(null)} className="px-4 py-2 border border-border text-text-primary text-sm font-bold rounded-md hover:bg-input transition-colors cursor-pointer">Close</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="bg-card border border-border rounded-lg p-4 shadow-sm mb-6">
        <div className="flex flex-wrap gap-4 items-end">
          ...
          <div className="flex flex-col">
            ...
            <input type="date" className="bg-input border border-border rounded-md py-2 px-3 text-sm font-medium text-text-primary focus:outline-none focus:border-primary transition-colors h-10" value={dateFrom} onChange={e => setDateFrom(e.target.value)} />
          </div>
          <div className="flex flex-col">
            <label className="text-xs font-bold text-text-secondary mb-1">To</label>
            <input type="date" className="bg-input border border-border rounded-md py-2 px-3 text-sm font-medium text-text-primary focus:outline-none focus:border-primary transition-colors h-10" value={dateTo} onChange={e => setDateTo(e.target.value)} />
          </div>
          <div className="flex-1 min-w-48 flex flex-col">
            <label className="text-xs font-bold text-text-secondary mb-1">Search</label>
            <input className="w-full bg-input border border-border rounded-md py-2 px-3 text-sm font-medium text-text-primary focus:outline-none focus:border-primary transition-colors h-10" placeholder="Student name or phone..." value={search} onChange={e => setSearch(e.target.value)} />
          </div>
        </div>
      </div>
