...
const TYPE_BADGE: Record<string, string> = {
  welcome: 'bg-[#1E3A5F] text-[#60A5FA]', fee_reminder: 'bg-[#451A03] text-[#FBBF24]',
  receipt: 'bg-[#064E3B] text-[#34D399]', notice: 'bg-[#312E81] text-[#818CF8]', renewal: 'bg-[#312E81] text-[#818CF8]',
};
const TYPE_LABEL: Record<string, string> = {
  welcome: 'Welcome', fee_reminder: 'Fee Reminder', receipt: 'Receipt', notice: 'Notice', renewal: 'Renewal',
};
const STATUS_BADGE: Record<string, string> = {
  Pending: 'bg-[#451A03] text-[#FBBF24]', Sent: 'bg-[#1E3A5F] text-[#60A5FA]', Delivered: 'bg-[#064E3B] text-[#34D399]', Failed: 'bg-[#450A0A] text-[#F87171]',
};
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-bg-card w-full max-w-lg rounded-xl shadow-2xl border border-border flex flex-col relative overflow-hidden">
            ...
            <div className="p-6">
              ...
              {viewLog.error && (
                <div className="mt-4 p-3 bg-[#450A0A] text-[#F87171] rounded-lg border border-[#F87171]/20 text-sm font-bold flex items-center gap-2"><AlertTriangle size={14} /> Error: {viewLog.error}</div>
              )}
            </div>
                  ...
                  {paginatedLogs.length > 0 ? (
                    paginatedLogs.map((log, index) => (
                      ...
                      >
                        ...
                        <TableCell>
                          <span className="text-[#F87171] font-medium text-xs truncate max-w-xs inline-block" title={log.error}>
                            {log.error || '—'}
                          </span>
                        </TableCell>
                      </TableRow>
