...
import { Category } from "./SuperadminNotificationCenterClient_types";
...
export function SuperadminNotificationCenterClient() {
      ...
      <div className="mb-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-text-secondary mt-1"><strong className="text-primary">{unread}</strong> unread notifications requiring your attention.</p>
          </div>
          <button onClick={markAllRead} className="flex items-center gap-2 px-4 py-2 text-sm font-bold text-text-secondary hover:text-text-primary bg-card hover:bg-input border border-border rounded-md transition-colors disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer shadow-sm" disabled={unread === 0}>
        ...
        <div className="flex flex-wrap gap-3">
          {CATS.map(( c ) => (
            <button key={c.id} onClick={() => setCat(c.id)}
              className={`flex items-center gap-2 px-4 py-2 text-sm font-bold rounded-full transition-all cursor-pointer ${
                cat === c.id 
        ...
        <div className="bg-card border border-border rounded-xl shadow-sm overflow-hidden flex flex-col">
          {filtered.length === 0 ? (
            <div className="flex flex-col items-center justify-center p-16 text-center text-text-secondary">
              ...
              <p className="text-lg font-extrabold text-text-primary">All caught up!</p>
              <p className="text-sm mt-2">No pending notifications in this category.</p>
            </div>
          ) : (
                ...
                <div key={n.id} className={`flex flex-col sm:flex-row sm:items-start gap-4 p-5 transition-colors hover:bg-input/30 ${n.read ? 'opacity-70 grayscale-[20%]' : 'bg-primary/[0.02]'}`}>
                  ...
                  <div className="flex-1 flex flex-col min-w-0">
                    <div className="flex flex-wrap items-center gap-2 mb-1.5">
                      <p className={`text-base font-extrabold truncate ${n.read ? 'text-text-primary' : 'text-primary'}`}>{n.title}</p>
                      
                      <span className={`px-2 py-0.5 text-xs font-bold uppercase tracking-wider rounded flex items-center gap-1 ${n.priority === 'High' ? 'bg-danger/10 text-danger' : 'bg-warning/10 text-warning'}`}>
                        <Circle size={8} fill="currentColor" /> {n.priority}
                      </span>
                      
                      {!n.read && <span className="px-2 py-0.5 text-xs font-bold uppercase tracking-wider rounded bg-primary text-primary-foreground animate-pulse shadow-sm">New</span>}
                    </div>
                    
                    <p className="text-sm text-text-secondary leading-relaxed mb-3">{n.description}</p>
