'use client';
// RESPONSIBILITY: Renders the SuperadminAutoSuspendClient component.
import React from 'react';
import { Toaster } from 'react-hot-toast';
import { Settings, Ban, RotateCcw, Bell, Save, UserCheck, ShieldAlert, X } from 'lucide-react';
import { useSuperadminAutoSuspendClient } from '@/app/superadmin/superadmin_finance/auto-suspend/_components/useSuperadminAutoSuspendClient';
import { TableToolbar } from "@/components/ui/table-toolbar";
import { useClientTable } from "@/components/ui/use-client-table";
import { TablePagination } from "@/components/ui/table-pagination";

export function SuperadminAutoSuspendClient() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <h1 className="text-xl font-bold text-text-primary">Auto-Suspend Policy</h1>
        <p className="text-xs text-text-secondary">Manage automatic suspension and student restoration.</p>
      </div>

      {/* KPI Cards */}
      <div className="grid gap-4 md:grid-cols-4">
        {KPI_CARDS.map(({ label, value, icon: Icon, variant }) => (
          <div key={label} className={`bg-card p-5 rounded-lg border ${variant === 'danger' ? 'border-danger/20' : 'border-border'} flex flex-col justify-between`}>
            <div className="flex items-center justify-between mb-2">
              <span className={`text-xs font-bold uppercase tracking-wider ${variant === 'danger' ? 'text-danger' : 'text-text-secondary'}`}>{label}</span>
              <Icon size={16} className={variant === 'danger' ? 'text-danger' : 'text-text-secondary'} />
            </div>
            <p className={`text-text-primary text-xl font-black tracking-tight ${variant === 'danger' ? 'text-danger' : 'text-text-primary'}`}>{value}</p>
          </div>
        ))}
      </div>
      ...
      <div className="bg-card rounded-lg border border-border p-6">
        ...
        {configLoading ? (
          ...
          <form onSubmit={onSaveConfig} className="space-y-4 max-w-sm">
            <div>
              <label className="text-xs font-bold text-text-secondary block mb-1">Days before auto-suspend <span className="text-danger">*</span></label>
              <input 
                type="number" 
                {...configForm.register('daysBeforeSuspend', { valueAsNumber: true })}
                className={`w-full bg-input border ${configForm.formState.errors.daysBeforeSuspend ? 'border-danger' : 'border-border'} rounded-md py-2 px-3 text-sm font-medium text-text-primary focus:outline-none focus:border-primary transition-colors`} 
              />
              {configForm.formState.errors.daysBeforeSuspend && <p className="text-danger text-xs mt-1">{configForm.formState.errors.daysBeforeSuspend.message}</p>}
              <p className="text-xs text-text-secondary mt-1">Students overdue beyond this period are automatically suspended.</p>
            </div>
            <div className="flex gap-2 pt-2">
              <button type="submit" className="flex items-center gap-1 bg-success text-success-foreground px-4 py-2 rounded-md text-xs font-bold hover:brightness-95 transition-all cursor-pointer disabled:opacity-50" disabled={updatePending}>
                <Save size={14} /> {updatePending ? 'Saving...' : 'Save Config'}
              </button>
              <button type="button" className="bg-transparent border border-border text-text-primary px-4 py-2 rounded-md text-xs font-bold hover:bg-input transition-colors cursor-pointer" onClick={cancelEditConfig}>Cancel</button>
            </div>
          </form>
        ) : (
          <p className="text-sm text-text-secondary bg-primary/5 p-4 rounded-md border border-primary/10">
            Students are automatically suspended after <strong className="text-text-primary text-base">{config?.daysBeforeSuspend}</strong> days of non-payment past due date.
          </p>
        )}
      </div>
      ...
      <div className="bg-card rounded-lg border border-border overflow-x-auto">
        <div className="flex items-center gap-2 p-4 border-b border-border bg-page/50">
          ...
          <span className="text-sm text-text-primary font-bold">Suspended Students</span>
        </div>
            ...
            {suspendedLoading ? (
              ...
              suspended.map((s) => (
                <tr key={s.id} className="border-b border-border last:border-0 hover:bg-primary/5 transition-colors">
                  ...
                  <td className="py-3 px-4 text-xs text-text-secondary">{s.suspendedSince}</td>
                  <td className="py-3 px-4">
      ...
      {restoreDialog && (
        <div className="fixed inset-0 z-40 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-md bg-card rounded-xl shadow-2xl overflow-hidden p-7 animate-in fade-in zoom-in-95 duration-200">
            ...
            <p className="text-sm text-text-secondary mb-4">Manually restore access for <strong className="text-text-primary">{restoreDialog.name}</strong>?</p>
            
            <form onSubmit={onRestoreStudent}>
              <div className="mb-6">
                <label className="text-xs font-bold text-text-secondary block mb-1">Override reason <span className="text-danger">*</span></label>
                <input 
                  {...restoreForm.register('restoreReason')} 
                  className={`w-full bg-input border ${restoreForm.formState.errors.restoreReason ? 'border-danger' : 'border-border'} rounded-md py-2.5 px-3 text-sm font-medium text-text-primary focus:outline-none focus:border-primary transition-colors`} 
                  placeholder="Enter reason..." 
                />
                {restoreForm.formState.errors.restoreReason && <p className="text-danger text-xs mt-1">{restoreForm.formState.errors.restoreReason.message}</p>}
              </div>
              <div className="flex justify-end gap-3">
                <button type="button" className="px-4 py-2 bg-transparent border border-border text-text-primary text-sm font-bold rounded-md hover:bg-input transition-colors cursor-pointer" onClick={() => setRestoreDialog(null)}>Cancel</button>
                <button
                  type="submit"
                  className="px-4 py-2 bg-success text-success-foreground text-sm font-bold rounded-md hover:brightness-95 transition-all cursor-pointer disabled:opacity-50 flex items-center gap-2"
