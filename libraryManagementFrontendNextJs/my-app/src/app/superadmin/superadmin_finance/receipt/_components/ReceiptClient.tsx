  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-xs text-text-secondary">View and share payment receipts for all transactions.</p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          <div key={k.label} className="bg-card rounded-lg border border-border p-4 relative overflow-hidden group">
            <div className="flex items-center justify-between mb-2 relative z-10">
              <span className={`text-xs font-bold uppercase tracking-wider ${k.success ? 'text-success' : 'text-text-secondary'}`}>{k.label}</span>
              <div className={`w-8 h-8 rounded-md flex items-center justify-center transition-transform duration-300 group-hover:scale-110 ${k.success ? 'bg-success/10 text-success' : 'bg-primary/10 text-primary'}`}>
                <Receipt size={16} />
              </div>
            </div>
            <p className={`text-3xl font-black tracking-tight relative z-10 ${k.success ? 'text-success' : 'text-text-primary'}`}>{k.value}</p>
          </div>
        ))}
      </div>
      ...
      <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-primary/5 uppercase text-xs font-semibold text-text-secondary border-b border-border">
              ...
              <th className="py-3 px-4 max-w-sm">Plan</th>
            ...
            {filtered.length === 0 ? (
              <tr>
                <td colSpan={7}>
                  <div className="flex flex-col items-center justify-center p-8 text-center space-y-3">
                    ...
                    <p className="text-base text-text-secondary">No receipts found.</p>
                  </div>
                </td>
              </tr>
              ...
              >
                <td className="py-3 px-4">
                  <span className="font-mono text-sm font-bold text-text-primary">{r.receiptNumber}</span>
                </td>
                <td className="py-3 px-4">
                  <div className="font-medium text-sm text-text-primary group-hover:text-primary transition-colors">{r.studentName}</div>
                  <div className="text-xs text-text-secondary">{r.studentId}</div>
                </td>
                <td className="py-3 px-4 text-xs text-text-secondary max-w-sm truncate">{r.planName}</td>
                <td className="py-3 px-4 text-xs text-text-secondary">{formatDate(r.date)}</td>
                <td className="py-3 px-4 text-right font-semibold text-sm text-text-primary">{formatCurrency(r.amount)}</td>
                <td className="py-3 px-4">
                  <span className={`${MODE_BADGE[r.paymentMode.toLowerCase()] || 'bg-info/10 text-info border-info/20'} px-2 py-0.5 rounded-full text-xs font-bold border capitalize`}>
                    {r.paymentMode}
                  </span>
                </td>
                <td className="py-3 px-4">
