...
import { shiftsSchema, type ShiftsData } from '@/app/superadmin/superadmin_shared_components/Superadminsuperadmin_schema';
...
export function SuperadminSetupWizardStep2({ onNext }: Props) {
  ...
  return (
    <form id="step2-form" onSubmit={handleSubmit(onNext)} noValidate className="space-y-3">
      {fields.map((field, i) => (
        <div key={field.id} className="grid gap-4 items-end bg-bg-pageg-card p-4 rounded-xl border border-border shadow-sm grid-cols-[1fr_120px_120px_40px] md:grid-cols-[1fr_140px_140px_40px]">
