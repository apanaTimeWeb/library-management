...
export function SetupWizardClient() {
  ...
  return (
    <div className="h-screen w-full flex bg-bg-pageg-base overflow-hidden relative">
      ...
      {showSkipModal && (
        <div className="fixed inset-0 bg-bg-pagelack/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-bg-pageg-card max-w-md w-full rounded-2xl shadow-2xl p-6 relative border border-border flex flex-col items-center text-center animate-in zoom-in-95 duration-200">
            ...
            <div className="w-full bg-bg-pageg-elevated rounded-xl p-4 text-left space-y-3 mb-6">
              {[
            ...
            <div className="flex w-full gap-3">
              <button
                ...
                className="px-4 py-2 rounded-lg text-sm font-medium transition-colors border border-border text-text-secondary bg-transparent hover:bg-bg-pageg-elevated hover:text-text-primary flex-1 py-2.5"
              >
                Continue Setup
              </button>
              <button
            ...
            <button
              type="button"
              onClick={() => setShowSkipModal(false)}
              className="absolute top-4 right-4 w-8 h-8 flex items-center justify-center rounded-full hover:bg-bg-pageg-elevated text-text-secondary hover:text-danger transition-colors"
            >
              <X size={15} />
            </button>
          </div>
      ...
      {/* ── STEPPER SIDEBAR ── */}
      <aside className="w-72 shrink-0 border-r border-border bg-bg-pageg-card/50 backdrop-blur flex-col p-6 z-10 hidden md:flex">
            ...
            return (
              <div key={s.num} className="relative">
                ...
                <div className={`flex gap-4 py-3 relative z-10 opacity-60 transition-opacity ${isActive ? 'opacity-100' : ''}`}>
                  <div className={`w-6 h-6 rounded-full border-2 bg-bg-pageg-card flex items-center justify-center transition-colors shrink-0 border-border ${isDone ? 'bg-primary border-primary' : isActive ? 'border-primary ring-4 ring-primary/20' : ''}`}>
                    {isDone
        ...
        <div className="mt-auto space-y-3">
          ...
          <button
            ...
            className="w-full py-2 flex items-center justify-center gap-2 text-sm font-medium text-text-tertiary hover:text-text-primary transition-colors hover:bg-bg-pageg-elevated rounded-md"
          >
      ...
      <main className="flex-1 flex flex-col">
        <div className="h-14 border-b border-border bg-bg-pageg-card/80 backdrop-blur flex items-center justify-between px-6 shrink-0 md:hidden">
        ...
        <div className="flex-1 overflow-y-auto p-6 md:p-10 relative z-10 scrollbar-thin">
          <div className="max-w-2xl mx-auto w-full">
            <div className="mb-8 animate-in fade-in slide-in-from-bottom-2 duration-500">
              ...
              <h2 className="text-text-primaryxl font-bold text-text-primary">{STEPS[step - 1].title}</h2>
            </div>
            ...
            {step === 5 && (
              <div className="space-y-5">
                <div className="p-6 rounded-xl border border-border bg-bg-pageg-card shadow-sm">
                ...
                <div className="p-4 rounded-xl border border-border bg-bg-pageg-card shadow-sm">
                  <p className="text-xs font-bold text-text-tertiary uppercase tracking-wider mb-3">Shifts</p>
                  <div className="flex flex-wrap gap-2">
                    {shifts.shifts.map(( s ) => (
                      <span key={s.name} className="text-xs font-medium text-text-secondary bg-bg-pageg-elevated px-2 py-1 rounded border border-border">
                ...
                <div className="p-4 rounded-xl border border-border bg-bg-pageg-card shadow-sm">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {plans.plans.map(( pl ) => (
                      <span key={pl.name} className="text-xs font-medium text-text-secondary bg-bg-pageg-elevated px-2 py-1 rounded border border-border">
            ...
            {step !== 5 ? (
              <div className="mt-8 pt-6 border-t border-border flex items-center justify-between">
                <button
                  ...
                  onClick={prevStep}
                  disabled={step === 1}
                  className="px-3 py-1.5 text-sm rounded-lg font-medium transition-colors border border-border text-text-secondary bg-transparent hover:bg-bg-pageg-elevated hover:text-text-primary disabled:opacity-30 disabled:cursor-not-allowed"
                >
