...
import {
  ...
  ENQUIRY_STATUS,
  COMPLAINT_STATUS,
} from '@/modules/manager/dashboard/manager/constants/manager.constants';
...
export class DashboardService {
  ...
  async getDashboardData(branchId?: string): Promise<DashboardResponse> {
    ...
    nextWeek.setDate(
      nextWeek.getDate() + MANAGER_CONSTANTS.EXPIRING_DAYS_THRESHOLD,
    );
    ...
    const kpiData = [
      {
        ...
        icon: 'Users',
        iconClass: 'mgr-kpi-icon--primary',
        trend: '↑',
      },
      {
        ...
        icon: 'CalendarCheck',
        iconClass: 'mgr-kpi-icon--success',
        trend: '',
      },
      {
        ...
        icon: 'Phone',
        iconClass: 'mgr-kpi-icon--info',
        trend: `${pendingEnquiries} new today`,
      },
      {
        title: 'Seats / Lockers Expiring (7 days)',
        value: expiringSubs.toString(),
        icon: 'Armchair',
        iconClass: 'mgr-kpi-icon--warning',
        trend: '',
      },
    ];
    ...
    const seatData = seats.map((seat, idx) => ({
      ...
      status:
        seat.status ||
        (idx % 3 === 0 ? 'available' : idx % 7 === 0 ? 'expiring' : 'occupied'),
    }));

    for (
      let i = seatData.length;
      i < MANAGER_CONSTANTS.MAX_SEATS_DISPLAY;
      i++
    ) {
    ...
    const actionItems = [
      ...
      },
      {
        ...
        countClass: 'mgr-action-count--warning',
      ...
      },
      {
        ...
        count: 0,
        countClass: 'mgr-action-count--warning',
        href: '/finance/payment-promises',
        showRenew: false,
      },
      {
        title: 'Maintenance / Broken Assets',
        count: 0,
        countClass: 'mgr-action-count--danger',
