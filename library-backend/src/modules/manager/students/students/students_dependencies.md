# Manager Students Module Dependencies

This document explicitly maps the dependencies of the `students` module within the `manager` domain, satisfying Rule 49 of the backend architecture guidelines.
...
- **App Module (`@/app.module`)**: Imports the `StudentsModule` to mount its routes on the global `/api/manager/students` prefix.
