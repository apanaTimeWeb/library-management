# Admin Students Module Dependencies

This document explicitly maps the dependencies of the `students` module within the `admin` domain, satisfying Rule 49 of the backend architecture guidelines.

## Upstream Dependencies (Modules this module depends on)

- **Auth Module (`@/modules/auth/auth.module`)**: Provides `JwtAuthGuard` and `RolesGuard` for securing student endpoints and enforcing RBAC.
...
- **App Module (`@/app.module`)**: Imports the `StudentsModule` to mount its routes on the global `/api/admin/students` prefix.
