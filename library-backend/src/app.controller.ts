...
export class AppController {
  ...
  async getComplaints() {
    return this.dataSource.query(
      'SELECT * FROM complaint ORDER BY "createdAt" DESC',
    );
  }
  ...
  async getPayments() {
    ...
    const exps = await this.dataSource.query(
  ...
  async getSeatMatrix() {
    return this.dataSource.query(
      'SELECT * FROM seat ORDER BY "seatNumber" ASC',
    );
  }
  ...
  async getLockers() {
    return this.dataSource.query(
      'SELECT * FROM locker ORDER BY "lockerNumber" ASC',
    );
  }
}
